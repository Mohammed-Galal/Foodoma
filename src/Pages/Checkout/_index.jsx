...
export default function () {
  ...
  if (!User.loaded || !Products.cart.length) return null;

  let accept2Continue = false,
    totalPrice = 0;

  // debugger;
  const isExceptionalCart = checkForExceptionalItems(Products.cart),
    order = Products.cart.map((CI) => {
  ...
  const calcSubtotalDelivery =
      Restaurant.data.free_delivery_subtotal > 0 &&
  ...
  return (
    <section id="checkout">
      ...
      <div className="align-items-stretch container d-flex flex-column flex-xl-row gap-3 justify-content-center">
        ...
        <OrderInfo
          aceeptCOD={delivery && !isExceptionalCart}
  ...
  function handleInvoice(res) {
    ...
    const basicOrderData = {
      ...
      restaurant_charge: Restaurant.data.restaurant_charges,
      paymentMode: paymentMethod === "COD" ? "عند الاستلام" : paymentMethod,
    };
...
function OrderInfo({
  ...
  const store = useStore().getState(),
    ...
    cashbackVal = +calcCashback(totalPrice, cashback),
      ...
      <div>
        ...
        <p>
          ...
          <span>
            {discount === false
              ? "جاري التحقق"
              : Math.abs(discount) + cashbackVal + " " + "ر.س"}
          </span>
        </p>
      </div>
      <hr />
...
const exceptionalCategories = [undefined, "الحجز المبكر"];
function checkForExceptionalItems(cartItems) {
  let result = false;

  cartItems.forEach(
    (i) => (result ||= exceptionalCategories.includes(i.category_name))
  );
